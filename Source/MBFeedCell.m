...
static CGFloat const InkwellFeedCellAvatarSize = 20.0;
static CGFloat const InkwellFeedCellTextLeading = 12.0;
static CGFloat const InkwellFeedCellTextSpacing = 4.0;
...
@property (nonatomic, strong) NSTextField* feedURLTextField;
...
- (void) configureWithSubscription:(MBSubscription*) subscription avatarImage:(NSImage*) avatar_image
{
	...
	self.siteURLTextField.stringValue = [self displayURLString:site_url_value];
	self.feedURLTextField.stringValue = [self displayURLString:feed_url_value];
	[self applyTextColors];
}
...
- (void) setupViews
{
	...
	[feed_url_text_field setContentCompressionResistancePriority:NSLayoutPriorityDefaultLow forOrientation:NSLayoutConstraintOrientationHorizontal];
	[feed_url_text_field setContentHuggingPriority:NSLayoutPriorityDefaultLow forOrientation:NSLayoutConstraintOrientationHorizontal];
	...
	text_stack_view.spacing = InkwellFeedCellTextSpacing;
	...
	[NSLayoutConstraint activateConstraints:@[
		...
		[text_stack_view.leadingAnchor constraintEqualToAnchor:avatar_image_view.trailingAnchor constant:InkwellFeedCellTextLeading],
...
- (NSString *) displayURLString:(NSString *)urlString
{
	NSString* display_url = [self trimmedString:urlString];
	NSString* lowercase_url = [display_url lowercaseString];
	if ([lowercase_url hasPrefix:@"https://"]) {
		display_url = [display_url substringFromIndex:8];
	}
	else if ([lowercase_url hasPrefix:@"http://"]) {
		display_url = [display_url substringFromIndex:7];
	}
