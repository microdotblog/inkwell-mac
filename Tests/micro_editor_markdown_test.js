...
const source = fs.readFileSync(path.join(__dirname, '..', 'Resources', 'NewPost', 'micro_editor.js'), 'utf8');
const node_types = { ELEMENT_NODE: 1, TEXT_NODE: 3 };
...
function textEditor(text) {
	return {
		...
		matches: () => false,
		get firstChild() {
			return { nodeType: node_types.TEXT_NODE, textContent: this.innerText, nextSibling: null };
		}
	};
}

const context = { console: console, Node: node_types };
vm.createContext(context);
vm.runInContext(source, context);

const formatEditorHTML = context.MicroEditor.formatEditorHTML;
...
test('saving does not remove a blank line after a code fence', () => {
	const markdown = '```\n\nintentional blank first line\n```';
	context.document = {
		getElementById: () => textEditor(markdown)
	};
	assert.equal(context.MicroEditor.getMarkdownByID('editor'), markdown);
});
...
test('autocomplete and local HTML conversion recognize complete usernames', () => {
	const listeners = {};
	const timers = new Map();
	const editor = textEditor('');
	editor.focus = () => {};
	editor.addEventListener = (name, callback) => { listeners[name] = callback; };
	const elements = { editor, toolbar: { contains: () => true }, editor_bold_button: {} };
	const sandbox = {
		console,
		Node: node_types,
		...
		showdown: { Converter: function() { this.makeHtml = text => text; } }
	};
	vm.runInNewContext(source, sandbox);
	let found;
	const instance = sandbox.MicroEditor.init({ textbox_id: 'editor', toolbar_id: 'toolbar', autocomplete_handler: username => { found = username; } });
	for (const mention of mentionExamples) {
		editor.innerText = `Hello ${mention}`;
		listeners.input({});
		timers.get(500)();
		assert.equal(found, mention);
		assert.equal(instance.getHTML(), `Hello <a href="https://micro.blog/${mention.slice(1)}">${mention}</a>`);
	}
	for (const suffix of ['.', ',', ' ', '!']) {
		editor.innerText = `Hello @bob_test${suffix}`;
		listeners.input({});
		timers.get(500)();
		assert.equal(found, '');
	}
...
// Inkwell uses DOM markers rather than zero-width characters for the caret.
test('preserves user-entered invisible characters and nonbreaking spaces', () => {
	const markdown = 'one\u200B\u200Btwo\u00A0three';
	assertRoundTrip(markdown);
	context.document = { getElementById: () => textEditor(markdown) };
	assert.equal(context.MicroEditor.getMarkdownByID('editor'), markdown);
});
